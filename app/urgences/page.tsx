'use client';

import React, { useMemo } from 'react';
import { useRouter } from 'next/navigation';
import { ChevronLeft, ShieldCheck } from 'lucide-react';
import { motion } from 'framer-motion';
import ContactCard from '../components/ContactCard';
import { CategoryType } from '@/types';
import { MOCK_CONTACTS } from '@/data';

export default function UrgencesPage() {
  const router = useRouter();

  const emergencyContacts = useMemo(() => {
    return MOCK_CONTACTS.filter(c => c.category === CategoryType.URGENCES);
  }, []);

  return (
    <motion.div
      className="max-w-6xl mx-auto px-4 md:px-8 py-12 md:py-16"
      initial={{ opacity: 0, y: 30 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.6, ease: 'easeOut' }}
    >
      {/* Header */}
      <motion.div
        className="flex items-center gap-6 mb-12"
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.2 }}
      >
        <button
          onClick={() => router.push('/')}
          className="p-4 bg-gray-50 hover:bg-gray-100 rounded-2xl transition-all active:scale-95"
        >
          <ChevronLeft size={24} />
        </button>

        <div>
          <h1 className="text-3xl font-black">Numéros d&apos;Urgence</h1>
          <p className="text-red-600 font-black text-[10px] uppercase tracking-widest">
            Accès gratuit illimité • 24h/7j
          </p>
        </div>
      </motion.div>

      {/* Grid contacts */}
      <motion.div
        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8"
        variants={{
          hidden: {},
          show: {
            transition: {
              staggerChildren: 0.12
            }
          }
        }}
        initial="hidden"
        animate="show"
      >
        {emergencyContacts.map(contact => (
          <motion.div
            key={contact.id}
            variants={{
              hidden: { opacity: 0, y: 20 },
              show: { opacity: 1, y: 0 }
            }}
            whileHover={{ scale: 1.03 }}
            transition={{ type: 'spring', stiffness: 200, damping: 15 }}
          >
            <ContactCard
              contact={contact}
              onCall={(p) => window.location.href = `tel:${p}`}
              onWhatsApp={(p) => window.open(`https://wa.me/${p}`, '_blank')}
            />
          </motion.div>
        ))}
      </motion.div>

      {/* Section alerte */}
      <motion.div
        className="mt-20 p-10 bg-red-50 rounded-[3rem] border border-red-100 text-center"
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ delay: 0.4, duration: 0.5 }}
      >
        <motion.div
          className="w-16 h-16 bg-red-600 text-white rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-xl shadow-red-200"
          animate={{ scale: [1, 1.05, 1] }}
          transition={{ duration: 1.5, repeat: Infinity }}
        >
          <ShieldCheck size={32} />
        </motion.div>

        <h3 className="text-xl font-black text-red-900 mb-4">
          Ces numéros sont gratuits
        </h3>
        <p className="text-red-700/60 font-medium max-w-xl mx-auto">
          Conformément à la réglementation béninoise, les appels vers les secours
          publics ne sont jamais décomptés de votre quota AllôBénin.
        </p>
      </motion.div>
    </motion.div>
  );
}
