...
function SearchContent() {
  ...
  const filteredContacts = useMemo(() => {
    return MOCK_CONTACTS.filter((c) => {
      const matchCategory =
        !categoryParam ||
        c.category === (categoryParam as CategoryType);
  ...
  const handleContactAction = (contact: Contact, action: () => void) => {
    ...
    const plan = PLANS[userState.plan as SubscriptionPlan];

    if (
      plan.limit !== -1 &&
      userState.dailyContacts.count >= plan.limit
    ) {
  ...
  const updateSearchParams = (params: { q?: string; category?: string }) => {
    ...
    if (params.q !== undefined) {
      params.q ? newParams.set('q', params.q) : newParams.delete('q');
    }
    ...
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-12 md:py-16">
      ...
      <div className="mb-12">
        <div className="flex items-center gap-6 mb-8">
          <button
            onClick={() => router.push('/')}
            className="p-4 bg-gray-50 hover:bg-gray-100 rounded-2xl"
          >
            <ChevronLeft size={24} />
          </button>
          ...
          <div>
            ...
            <p className="text-blue-600 font-black text-[10px] uppercase tracking-widest">
              Quota :{' '}
              {userState
                ? PLANS[userState.plan].limit === -1
                  ? 'Illimité'
                  : `${userState.dailyContacts.count} / ${PLANS[userState.plan].limit}`
                : '...'}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
          <button
            ...
            className={`px-6 py-2.5 rounded-2xl text-sm font-black ${
              !categoryParam
          ...
          {CATEGORIES.map((cat) => (
            <button
              key={cat.id}
              onClick={() =>
                updateSearchParams({ category: cat.type })
              }
              className={`px-6 py-2.5 rounded-2xl text-sm font-black flex items-center gap-2 ${
                categoryParam === cat.type
                  ? 'bg-blue-600 text-white'
                  : 'bg-gray-50 text-gray-400'
              }`}
            >
